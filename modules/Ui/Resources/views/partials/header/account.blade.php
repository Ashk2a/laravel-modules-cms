...
<x-ui::header.dropdown.container class="right-0" x-show="account === true" @click.away="account = false">
    <ul class="flex justify-between flex-col w-full py-3 text-white">
        @guest
            <x-ui::header.dropdown.banner :route="route('auth.login')" bg="lightBlue">@lang('ui::global.login')</x-ui::header.dropdown.banner>
        ...
        @auth
            @can('manager.dashboard.index')
                <x-ui::header.dropdown.banner class="mb-3" :route="route('home')" bg="red">Manager</x-ui::header.dropdown.banner>
            @endcan

            <x-ui::header.dropdown.banner :route="route('home')" bg="lightBlue">@lang('ui::global.see_my_profile')</x-ui::header.dropdown.banner>
            <x-ui::header.dropdown.separator/>
            <x-ui::header.dropdown.category>My panel</x-ui::header.dropdown.category>
            <x-ui::header.dropdown.item :route="route('panel.overview')">Overview</x-ui::header.dropdown.item>
            <x-ui::header.dropdown.item :route="route('panel.preferences')">Preferences</x-ui::header.dropdown.item>
            <x-ui::header.dropdown.separator/>
