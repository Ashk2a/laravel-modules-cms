@if($root->canShowed())
    <div
        ...
        class="group flex justify-center items-center w-full border-transparent border-b-2"
    >
        <a
            href="{{ $root->getUrl() }}"
            ...
            class="flex inline-flex justify-center items-center w-full py-[1.5rem] text-sm xl:text-lg font-medium group-hover:text-gray-50"
        >
            <span>{{ $root->name }}</span>
    ...
    @if(false === $root->categories->isEmpty())
        <div
            ...
            @mouseleave="open = false"
            x-show="open === {{ $root->id }}"
            class="absolute xl:top-[4.80rem] top-[4.3rem] p-6 w-full grid grid-cols-2 xl:gap-x-10 gap-x-6 bg-gradient-to-b from-brown-600-95a to-brown-600"
        >
