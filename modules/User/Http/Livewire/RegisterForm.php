...
class RegisterForm extends AbstractFormComponent
{
    ...
    {
        return view('user::livewire.register-form');
    }
    ...
    public function submit(AuthService $authService): Redirector|RedirectResponse|null
    {
        ...
        if (null !== $user) {
            $this->flashNextSuccess(trans('user::text.verification_email_sent', ['email' => $user->email]));

            return redirect()->route('auth.login');
        }

        $this->flashNowDanger(trans('user::text.cannot_create_account'));
